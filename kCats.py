...
class inferenceModel(pints.ForwardModel):
    ...
    def simulate(self, params, times):
        params = np.multiply(np.asarray([2694.91, 400, 12, 0.0000027, 12, 400/60, 33/60, 58.8/60, 400/60, 400000/18000, 400000/18000, 400000/18000, 400000/18000, 400000/18000]), np.hstack([np.ones(4), params, np.ones(5)]))
...
if __name__ == '__main__':
    ...
    # mode = 0 because we want to fit kCats to model with ERAP!
    model = inferenceModel(mode=0)
    ...
    try:
        ...
        np.savetxt('data/kCatsData.txt', noisy_data)

    # fit to this data using MCMC

    default_params_noise = np.hstack([default_params, 0.1]) # we add a new unknown (sigma) for the multiplicative noise
    ...
    for i in range(3):
        np.savetxt('results/kCats_' + str(i + 1), chains[i, :, :])
