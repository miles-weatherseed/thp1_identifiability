...
class inferenceModel(pints.ForwardModel):
    ...
    def simulate(self, params, times):
        params = np.multiply(np.asarray([2694.91, 400, 12, 0.0000027, 12, 400/60, 33/60, 58.8/60, 400/60, 400000/18000, 400000/18000, 400000/18000, 400000/18000, 400000/18000]), np.hstack([1, params[0], np.ones(2), params[1:], np.ones(5)]))
...
if __name__ == '__main__':
    ...
    try:
        ...
        np.savetxt('data/kCatsE1Data.txt', noisy_data)
    ...
    for i in range(3):
        np.savetxt('results/kCatsE1_' + str(i + 1), chains[i, :, :])
